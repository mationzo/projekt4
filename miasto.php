		...
		<table>
			...
			<?php
				...
				if (isset($_POST['delete_id'])) {
					...
					$sql = "DELETE FROM miasto WHERE ID_m = $delete_id";
				...
				if ($result = mysqli_query($do_bazy, 'SELECT * FROM miasto')) {
				  while ($row = mysqli_fetch_assoc($result)) {
				    ...
				    if (isset($_POST['edit_id']) && $_POST['edit_id'] == $row['ID_m']) {
				      ...
				      echo "</td>";
				      echo "<td><div class='divprzycisk'><button type='submit'>Zapisz</button></td></div>";
				      echo "</form>";
				    } else {
				      echo $row['Nazwa'];
				      echo "</td>";
				      ...
				      echo "</form>";
					  echo "<form method='POST' action='miasto.php' onsubmit='return confirm(\"Czy na pewno chcesz usunąć ten rekord?\")'>";
					  echo "<input class='usun-przycisk' type='hidden' name='delete_id' value='" . $row['ID_m'] . "'>";
